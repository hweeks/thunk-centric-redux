...
function createRecursionDetectThunkMiddleware<TReturnType, TState, TBasicAction extends Action = AnyAction, TExtraThunkArg = undefined>(extraArgument?: TExtraThunkArg) : ThunkMiddleware<TState, TBasicAction, TExtraThunkArg> {
  return ({ dispatch, getState }) => {
    return next => (action: ThunkAction<TReturnType, TState, TExtraThunkArg, TBasicAction>) => {
      if (typeof action === 'function') {
        return action(wrappedDispatch, getState, extraArgument);
      }
      return next(action);
    };
    function wrappedDispatch(action: ThunkActionWithSymbol<TReturnType, TState, TExtraThunkArg, TBasicAction>) {
      if (typeof action === 'function') {
...
export const isNestedThunkSymbol = Symbol('isNestedThunk') as unknown as 'isNestedThunk';
...
export interface ThunkActionWithSymbol<TReturnType, TState, TExtraThunkArg, TBasicAction extends Action> extends ThunkAction<TReturnType, TState, TExtraThunkArg, TBasicAction> {
  isNestedThunk?: boolean
}

/**
 * A generic type that creates a standard middleware while also optionally adding an
 * extraArg augmentation to the default.
 *
 * @template TState The redux state
 * @template TBasicAction The (non-thunk) actions that can be dispatched
 * @template TExtraThunkArg An optional extra argument to pass to a thunk's
 * inner function. (Only used if you call `thunk.withExtraArgument()`)
 */
export type ThunkMiddlewareWithExtraArgs<
  TState = {},
  TBasicAction extends Action = AnyAction,
  TExtraThunkArg = undefined
>  = ThunkMiddleware<
...
const thunk = createRecursionDetectThunkMiddleware() as unknown as ThunkMiddlewareWithExtraArgs;
